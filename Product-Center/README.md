...
- **阶段**：原型设计 + PRD 文档
- **最近更新**：2026-09-08 — VPBC shell 新增**账户管理** Tab（份额信息下方）：维护基金账户信息与账户用途（托管户/货期存款/定期存款/券商结算/费用户/股息代收），含开户行、账户名称、账户号、SWIFT/BIC、币种、开户日期、状态等开户基础信息；列表筛选（基金/用途/开户行/状态）+ 新增/修改弹窗（选基金自动带出名称、标签控件同行）
- **下一步**：份额类别详情/编辑弹层 / 产品生命周期页面 / 费率结构页面 / 账户管理 PRD / 产品主数据 PRD 评审
...
- [x] 2026-09-08 VPBC shell 新增「账户管理」Tab（份额信息菜单下）：基金账户信息 + 账户用途 + 开户基础信息（开户行/账户名称/账户号/SWIFT/币种/开户日期/状态），9 条示例数据含「待开立」状态，新增/修改弹窗按标签控件同行铁律
