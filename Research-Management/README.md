...
- **最近更新**：2026-09-08 — VPBC shell「行业配置」更名**个股覆盖**：单只股票唯一归属一个研究员，覆盖股票个数/明细/代码模糊搜索均按研究员归属口径；新增/修改弹窗（研究员单选默认登录人、市场多选非必填、覆盖股票支持模糊搜索+文件导入）；新建研报：报告类型改为评级报告/非评级报告、展望改名评级（买入/卖出/维持）、删除 AAA 评级字段
- **下一步**：指标库"保存展示设置"落地（localStorage）；PRD 文档同步原型新功能（研报列表/表单的评级口径、个股覆盖模型）；考虑恢复公开预览链接
...
- [x] 2026-09-08 VPBC shell「行业配置」→「个股覆盖」：股票单归属研究员模型（个数/明细/模糊搜索同口径）；筛选条在上、新增/修改/删除按钮在下；新增/修改弹窗（研究员单选默认当前登录人、市场非必填多选、覆盖股票=手工模糊搜索+文件导入）；A股代码后缀统一 .CH
- [x] 2026-09-08 新建/修改研报：报告类型单选改为 评级报告/非评级报告（非评级时禁用评级）；「展望」改名「评级」（买入/卖出/维持）；删除 AAA 式评级字段
